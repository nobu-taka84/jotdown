...
BEGIN
    ...
    COMMENT ON TABLE item IS 'アイテム';
    COMMENT ON COLUMN item.id IS 'ID';
    COMMENT ON COLUMN item.user_info_id IS 'ユーザーID';
    COMMENT ON COLUMN item.title IS 'タイトル';
    COMMENT ON COLUMN item.memo IS 'メモ';
    COMMENT ON COLUMN item.sortorder IS 'ソートオーダー';
    COMMENT ON COLUMN item.updated_at IS '更新日';
    COMMENT ON COLUMN item.updated_by IS '更新者';
    COMMENT ON COLUMN item.created_at IS '作成日';
    COMMENT ON COLUMN item.created_by IS '作成者';
    COMMENT ON COLUMN item.version IS 'version';
